...
export async function vpCreatePerson(c: Context): Promise<Response> {
	...
	// Step 1: Register (creates Auth + User with null profile)
	...
	if (!registerResult.success) return resultToResponse(c, registerResult, 201);

	const { userId, token } = registerResult.value;

	// Step 2: Update profile with name and phone
	...
	if (!updateResult.success) {
		// Registration succeeded but profile update failed — return registration result
		return c.json({ success: true, data: { userId, token } }, 201);
	}

	return c.json({
		success: true,
		data: { ...toVpPerson(updateResult.value), token },
	}, 201);
}
