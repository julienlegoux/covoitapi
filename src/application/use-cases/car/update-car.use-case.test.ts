...
describe('UpdateCarUseCase', () => {
	...
	const existingCar = { id: 'car-1', refId: 1, licensePlate: 'AB-123-CD', modelRefId: 10 };
	...
	it('should update car with licensePlate only', async () => {
		...
		const result = await useCase.execute('car-1', { licensePlate: 'XY-999-ZZ' });
	...
	it('should update car with model resolution (existing model)', async () => {
		...
		const result = await useCase.execute('car-1', { model: 'Yaris', brandId: 'brand-1' });
